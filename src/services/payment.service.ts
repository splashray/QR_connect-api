...
//     const response: AxiosResponse = await paypalAxios.post(
...
const PAYPAL_API_URL = "https://api-m.sandbox.paypal.com/v1/billing/subscriptions"; // Use the sandbox URL for testing

class PaypalService {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  static async createSubscription(payload: any): Promise<any> {
    // eslint-disable-next-line no-useless-catch
    try {
      const response = await axios.post(PAYPAL_API_URL, payload, {
        headers: {
          Authorization: "Bearer " + process.env.PAYPAL_ACCESS_TOKEN,
          "Content-Type": "application/json",
          Accept: "application/json",
